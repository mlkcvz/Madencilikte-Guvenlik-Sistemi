...
export default class Reports extends React.Component {
  ...
  state = {
    ...
    user: '',
    loading: false,
    //ekrana yazılacak rapor değerleri
  ...
  componentDidMount = () => {
    ...
    var oneDay = (timestamp - 84000000); //şu anki tarihten 24 saate karşılık ms çıkar, bir gün öncesini bul
    var min = 9999999, max = 0, avr, tmp, sum = 0, count = 0;


    // CO2 günlük rapor
    firebase.database().ref('co2').orderByChild('timestamp').startAt(oneDay).on('value', (snap) => {
      let datas = [] //bir gün önceden başlayarak tüm verileri datas dizisine çek
      snap.forEach((item) => {
        const { data } = item.val();
        tmp = parseInt(data); //datayı tmp değişkenine al, en büyük ve en küçüğü çek.
        if (tmp > max) { 
          max = tmp;
          console.log(' tmp degeri ::::: ' + tmp);
        }
        if (tmp < min) {
          min = tmp;
        }
        sum += tmp; //dataların toplamı
        count++; //data sayısı
        avr = Math.round(sum / count); //ortalama
        datas.push({ data });
      });
      if(min == 9999999) { //veri gelmediyse 9999999 değeri değişmemiştir. //BERBAT ÇÖZÜM! BURAYA BAK !!! 
        this.setState({ co2max: '--' });
      this.setState({ co2min: '--' });
      this.setState({ co2avr: '--' });
      } else {
    ...
    firebase.database().ref('o2').orderByChild('timestamp').startAt(oneDay).on('value', (snap) => {
      let datas = []
      snap.forEach((item) => {
        const { data } = item.val();
        tmp = parseInt(data);
    ...
    //CO gnlük rapor
    min = 9999999, max = 0, avr, tmp, sum = 0, count = 0;
    firebase.database().ref('co').orderByChild('timestamp').startAt(oneDay).on('value', (snap) => {
      let datas = []
      snap.forEach((item) => {
        const { data } = item.val();
        tmp = parseInt(data);
    ...
    //CH4 günlük rapor
    min = 9999999, max = 0, avr, tmp, sum = 0, count = 0;
    firebase.database().ref('ch4').orderByChild('timestamp').startAt(oneDay).on('value', (snap) => {
      let datas = []
      snap.forEach((item) => {
        const { data } = item.val();
        tmp = parseInt(data);
      ...
      if(min == 9999999) {
        this.setState({ ch4max: '--' });
      this.setState({ ch4min: '--' });
      this.setState({ ch4avr: '--' });
      } else {
    ...
    min = 9999999, max = 0, avr, tmp, sum = 0, count = 0;
    firebase.database().ref('temperature').orderByChild('timestamp').startAt(oneDay).on('value', (snap) => {
      let datas = []
      snap.forEach((item) => {
        const { data } = item.val();
        tmp = parseInt(data);
      ...
      if(min == 9999999) {
        this.setState({ tempaturemax: '--' });
      this.setState({ tempaturemin: '--' });
      this.setState({ tempatureavr: '--' });
      } else {
    ...
    min = 9999999, max = 0, avr, tmp, sum = 0, count = 0;
    firebase.database().ref('pressure').orderByChild('timestamp').startAt(oneDay).on('value', (snap) => {
      let datas = []
      snap.forEach((item) => {
        const { data } = item.val();
        tmp = parseInt(data);
      ...
      if(min == 9999999) {
        this.setState({ pressuremax: '--' });
      this.setState({ pressuremin: '--' });
      this.setState({ pressureavr: '--' });
      } else {
    ...
    min = 9999999, max = 0, avr, tmp, sum = 0, count = 0;
    firebase.database().ref('humidity').orderByChild('timestamp').startAt(oneDay).on('value', (snap) => {
      let datas = []
      snap.forEach((item) => {
        const { data } = item.val();
        tmp = parseInt(data);
      ...
      if(min == 9999999) {
        this.setState({ humiditymax: '--' });
      this.setState({ humiditymin: '--' });
      this.setState({ humidityavr: '--' });
      } else {
  ...
  render() {
    return (
      ...
      <Text>Günlük Rapor Görüntüle </Text>
      </View>
      <View style={styles.panel}>
