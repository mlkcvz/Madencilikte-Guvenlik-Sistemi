...
export default class Reports extends React.Component {
  ...
  componentDidMount = () => {
    ...
    var oneWeek = (timestamp - 604800000);
    var min = 9999999, max = 0, avr, tmp, sum = 0, count = 0;


    // CO2 günlük rapor
    firebase.database().ref('co2').orderByChild('timestamp').startAt(oneWeek).on('value', (snap) => {
      let datas = []
      snap.forEach((item) => {
        const { data } = item.val();
        tmp = parseInt(data);
    ...
    firebase.database().ref('o2').orderByChild('timestamp').startAt(oneWeek).on('value', (snap) => {
      let datas = []
      snap.forEach((item) => {
        const { data } = item.val();
        tmp = parseInt(data);
    ...
    //CO gnlük rapor
    min = 9999999, max = 0, avr, tmp, sum = 0, count = 0;
    firebase.database().ref('co').orderByChild('timestamp').startAt(oneWeek).on('value', (snap) => {
      let datas = []
      snap.forEach((item) => {
        const { data } = item.val();
        tmp = parseInt(data);
    ...
    //CH4 günlük rapor
    min = 9999999, max = 0, avr, tmp, sum = 0, count = 0;
    firebase.database().ref('ch4').orderByChild('timestamp').startAt(oneWeek).on('value', (snap) => {
      let datas = []
      snap.forEach((item) => {
        const { data } = item.val();
        tmp = parseInt(data);
    ...
    min = 9999999, max = 0, avr, tmp, sum = 0, count = 0;
    firebase.database().ref('temperature').orderByChild('timestamp').startAt(oneWeek).on('value', (snap) => {
      let datas = []
      snap.forEach((item) => {
        const { data } = item.val();
        tmp = parseInt(data);
    ...
    min = 9999999, max = 0, avr, tmp, sum = 0, count = 0;
    firebase.database().ref('pressure').orderByChild('timestamp').startAt(oneWeek).on('value', (snap) => {
      let datas = []
      snap.forEach((item) => {
        const { data } = item.val();
        tmp = parseInt(data);
    ...
    min = 9999999, max = 0, avr, tmp, sum = 0, count = 0;
    firebase.database().ref('humidity').orderByChild('timestamp').startAt(oneWeek).on('value', (snap) => {
      let datas = []
      snap.forEach((item) => {
        const { data } = item.val();
        tmp = parseInt(data);
  ...
  render() {
    return (
            ...
            <Text>Haftalık Rapor Görüntüle </Text>
            </View>
            <View style={styles.panel}>
